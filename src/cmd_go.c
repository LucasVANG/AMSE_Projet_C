...
void cycl_alm_handler( int signal ) //On lit la mémoire partagé a chaque itération
{
    ...
    double command = min(1.0,distance); 
    ...
    *tv_L = command;
    ...
    printf("Command : %.2f -- Dist : %.2f\n",command,dist(coord,&goal));
}
...
int main( int argc, char *argv[])
{
    ...
    if( sscanf(argv[1], "%lf", &distanceOrder ) == 0){return(0);};
    ...
    // Initiate Go Order
    goal.x = coord->x + sin(coord->theta)*distanceOrder;
    goal.y = coord->y + cos(coord->theta)*distanceOrder;
    lastDistance = dist(coord,&goal) + 1;
